vim.api.nvim_create_user_command("ZichuanCheckIcons", function()
  ...
  local line = ""
  local item_number = 0
  for name, icon in require("core.utils").ordered_pair(Zichuan.symbols) do
  ...
  if dir ~= nil then
    ...
    while true do
      ...
      if item_type == "directory" and item ~= "readme" then
        ...
        vim.system({ "git", "log", "-1", "--format=%cd", "--date=short" }, { cwd = plugins_path .. item }, function(obj)
...
vim.api.nvim_create_user_command("ZichuanRepeat", function(args)
